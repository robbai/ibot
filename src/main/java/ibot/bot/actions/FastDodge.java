...
public class FastDodge extends Action {
	...
	public ControlsOutput getOutput(DataPacket packet){
		...
		if(timeElapsed < TIMING[0]){
			...
			if(packet.car.hasDoubleJumped) return controls;
		...
		return new ControlsOutput().withOrient(AirControl.getRollPitchYaw(bot.car, directionGlobal)).withJump(false).withBoost(boost);
	}
