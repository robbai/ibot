...
public class FollowArcs extends Action {
	...
	public ControlsOutput getOutput(DataPacket packet){
		for(int i = 0; i < N - 1; i += RENDER_STEP){
			this.bot.renderer.drawLine3d(this.bot.altColour, this.points[i].withZ(Constants.CAR_HEIGHT), this.points[Math.min(N - 1, i + RENDER_STEP)].withZ(Constants.CAR_HEIGHT));
		}
...
//		double targetVelocity = Constants.SUPERSONIC_VELOCITY;
		...
		if(carUu < this.compositeArc.getL(0)){
			...
			targetVelocity = MathsUtils.lerp(Constants.SUPERSONIC_VELOCITY, DrivePhysics.getSpeedFromRadius(Math.abs(this.compositeArc.getR2())), (carUu - this.compositeArc.getL(0)) / this.compositeArc.getL(1));
		}else{
...
//			targetVelocity = Constants.SUPERSONIC_VELOCITY;
		...
		if(packet.ball.latestTouch != null && packet.ball.latestTouch.elapsedSeconds == packet.ball.time){
			if(packet.ball.latestTouch.playerIndex == bot.getIndex()){
				bot.sendQuickChat(progress > 0.25 ? QuickChatSelection.Apologies_Whoops : QuickChatSelection.Custom_Toxic_404NoSkill);
		...
		this.lastTime += dt;
		this.PRESSURE_UU = MathsUtils.clamp(this.PRESSURE_UU + PRESSURE_RATE * dt, carUu + AIM_UU, this.compositeArc.getLength() + AIM_UU);
		...
		boolean onStraightaway = (carUu >= this.compositeArc.getL(0) + this.compositeArc.getL(1) && carUu <= this.compositeArc.getL(0) + this.compositeArc.getL(1) + this.compositeArc.getL(2));
		boolean canDodge = onStraightaway && carUu + DrivePhysics.estimateDodgeDistance(car) < this.compositeArc.getL(0) + this.compositeArc.getL(1) + this.compositeArc.getL(2) + this.compositeArc.getL(3);

		Output output = Handling.driveVelocity(this.bot, target.withZ(Constants.CAR_HEIGHT), canDodge, false, targetVelocity);
	...
	private double carDistance(Vector2 car){
		if(this.points.length == 1) return this.points[0].distance(car);
	...
	private boolean onPath(Car car, double carUu){
		...
		if(this.points == null || carDistance(carPosition) > 500) return false;
