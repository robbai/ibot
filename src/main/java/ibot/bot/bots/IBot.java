...
public class IBot extends DataBot {
	...
	protected ControlsOutput processInput(DataPacket packet){
		...
		if(this.mode != Mode.SOCCAR || !this.isKickoff){
			if(this.aerialDodge != null) renderer.drawLine3d(Color.BLACK, this.carPosition, this.aerialDodge.position);
			if(this.aerialDouble != null) renderer.drawLine3d(Color.WHITE, this.carPosition, this.aerialDouble.position);
		}
		...
		if(this.commit && (!this.isKickoff || (car.forwardVelocity > 1300 && this.mode != Mode.SOCCAR))){
			...
			if(this.isKickoff || !this.car.hasWheelContact){
				...
				doubleJump = (localDouble.z > 550 && localDouble.normalised().z > MathsUtils.lerp(0.6, 0.3, Math.pow(this.carSpeed / Constants.MAX_CAR_VELOCITY, 2))
						|| (this.aerialDouble.position.z > Constants.GOAL_HEIGHT - 50 && Math.abs(this.aerialDouble.position.x) < Constants.GOAL_WIDTH + 200 && Math.abs(this.aerialDouble.position.y) < -Constants.PITCH_LENGTH_SOCCAR + 1100));
			}else{
				doubleJump = (this.aerialDodge == null);
			}
			//			if(!this.kickoff) doubleJump = (this.aerialDodge == null || new Spherical(MathsUtils.local(car, this.aerialDodge.ballPosition)).getElevation() > Math.toRadians(55));
			...
			if(aerialIntercept != null){
				...
				if(betterThanDriveStrike && Math.abs(radians) < Math.toRadians(doubleJump ? 35 : 45) * (this.goingInHomeGoal ? 1.5 : 1) && 
					...
					this.action = new Aerial(this, aerialIntercept, type).withAbortCondition(new BallTouchedAbort(this, packet.ball.latestTouch, this.playerIndex), new SliceOffPredictionAbort(this, aerialIntercept));
		...
		if(this.commit){
			...
			Vector3 dodgeTarget = this.groundIntercept.intersectPosition.lerp(this.groundIntercept.position, 0.3);
			if(this.wallIntercept != null){
				target = this.wallIntercept.intersectPosition;
				//    				if(target.y * this.sign > 0 && Math.abs(this.ballPosition.x) > Math.abs(target.x)){
				...
				if(Math.abs(car.position.y) < Constants.PITCH_LENGTH_SOCCAR || Math.abs(target.x) < Constants.GOAL_WIDTH){
					target = target.plus(Vector3.Y.scaleToMagnitude(-this.sign * target.distance(carPosition) * Math.min((this.bounce.time - this.time) / 2.5, 0.4)));
				}
				...
				if(this.mode == Mode.DROPSHOT){
					...
					target = target.plus(offset.scaleToMagnitude(-this.sign * target.distance(carPosition) * (this.isKickoff ? 0.05 : 0.2)));
					...
					if((this.groundIntercept.position.y - this.car.position.y) * this.sign < 0){
						...
						Vector2 trace = this.car.position.flatten().plus(toIntercept.scale((-this.sign * Constants.PITCH_LENGTH_SOCCAR) / toIntercept.y));
...
//						}
					...
					if(addedOffset > 0.001){
					...
					}else if(MathsUtils.local(this.car, this.groundIntercept.position).z > 160 && this.car.hasWheelContact){
						...
						if(Math.abs(this.carForwardComponent) > 0.975 /*&& Math.abs(radians) < Math.toRadians(50)*/){
							this.action = new DriveStrike(this, this.groundIntercept.withIntersectPosition(dodgeTarget)).withAbortCondition(new BallTouchedAbort(this, packet.ball.latestTouch, this.playerIndex), new SliceOffPredictionAbort(this, this.groundIntercept));
							return this.action.getOutput(packet);
						}
					}
				}else{
				...
				this.renderer.drawLine3d(this.altColour, this.groundIntercept.intersectPosition, this.groundIntercept.position);
				this.renderer.drawRectangle3d(this.colour, this.groundIntercept.intersectPosition, 8, 8, true);
				if(this.car.hasWheelContact && this.getTimeOnGround() > 0.2 && (this.groundIntercept.time - this.time) < 0.3
						&& Math.abs(this.lastControls.getSteer()) < (this.goingInHomeGoal ? 0.4 : 0.2)){
					//					boolean opponentBlocking = false;
					//					for(Car car : packet.enemies){
					//						if(this.groundIntercept.position.minus(this.car.position).angle(car.position.minus(this.car.position)) < Math.toRadians(30)){
					//							opponentBlocking = true;
					//							break;
					//						}
					//					}
					if(/*Math.abs(localInterceptBall.z) > 100 || */Math.abs(this.groundIntercept.time - this.earliestEnemyIntercept.time) < 0.15 || this.carSpeed < (this.car.onFlatGround ? 1100 : 1550)){
			...
			//			Intercept enemyIntersect = (this.furthestBack ? null : this.enemyIntersect());
...
//				target = enemyIntersect.position;
			}else if((this.car.boost < 40 || this.isKickoff) && this.mode != Mode.DROPSHOT && this.nearestBoost != null){
				...
				if(this.nearestBoost.isFullBoost() || this.mode == Mode.HOOPS){
					//					if((this.car.onSuperFlatGround && this.carForwardComponent > 0.975 && !this.lastControls.holdHandbrake()) || this.isKickoff){
					//						Vector2 endTarget = (this.teamPossession >= -0.001 || this.isKickoff ? target.flatten().withY(0) : this.homeGoal.flatten()/*.multiply(new Vector2(-1, 1))*/);
					//
					//						//						Vector2 endTarget;
					//						//						Vector2 direction = target.minus(this.carPosition).flatten().normalised();
					//						//						if(Math.abs(target.y) < 1000){
					//						//							endTarget = (direction.y * this.sign > 0 ? this.groundIntercept.intersectPosition.flatten() : this.homeGoal.flatten().multiply(new Vector2(-1, 1)));
					//						//						}else{
					...
					//						//						}
					//
					//						CompositeArc compositeArc = CompositeArc.create(this.car, target.flatten(), endTarget, 1000, 200, 300);
					//
					...
					//						//							}
					//						//							CompositeArc arcs = CompositeArc.create(this.car, target.flatten(), endTarget, 1200, 150, 300);
					//						//							if(compositeArc == null || arcs.getLength() < compositeArc.getLength()){
					//						//								compositeArc = arcs;
					//						//							}
					//						//						}
					//
					//						this.action = new FollowArcs(this, compositeArc).withAbortCondition(new BoostYoinkedAbort(this, this.nearestBoost));
					//						if(!this.isKickoff){
					...
					if(offset.dot(target.flatten()) < 0) offset = offset.scale(-1);
				...
				//    			target = packet.ball.position.withX(this.groundIntercept.interceptPosition.x * (this.mode == Mode.SOCCAR ? 0.4 : 0.75))
				//    					.withY(Math.copySign(packet.ball.position.y * MathsUtils.lerp(0.9, 0.45, car.boost / 100), -car.sign));
				//    			}
			}else{
				dontBoost = this.car.hasWheelContact;

				final double MIN_DEPTH = 0.35;
				final double MAX_DEPTH = 0.8;
				double depthLerp = MathsUtils.clamp((this.teamPossession * 0.5) + 0.2, 0, 1);
				this.stackRenderString("Depth: " + MathsUtils.round(depthLerp), this.colour);

				//				target = this.homeGoal.lerp(this.earliestEnemyIntercept.position, MathsUtils.lerp(MIN_DEPTH, MAX_DEPTH, depthLerp)).withZ(Constants.CAR_HEIGHT);
...
//				direction = direction.plus(this.earliestEnemyIntercept.car.velocity.flatten().scale(0.3 / Constants.SUPERSONIC_VELOCITY));
				target = this.earliestEnemyIntercept.position.plus(direction.scaleToMagnitude(goalDistance * (1 - MathsUtils.lerp(MIN_DEPTH, MAX_DEPTH, depthLerp))));
...
//					CompositeArc compositeArc = CompositeArc.create(this.car, target.flatten(), endTarget, 1300, 200, 300);
//					this.action = new FollowArcs(this, compositeArc).withBoost(false).withAbortCondition(new CommitAbort(this, 0.1));
//					return action.getOutput(packet);
//				}
			}
		}
		Output output = Handling.driveTime(this, target, (!this.isKickoff || this.mode == Mode.SOCCAR && !wall), this.mode == Mode.DROPSHOT || !this.commit, targetTime);
		if(output.isAction()){
