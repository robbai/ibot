...
public class JumpPhysics {
	...
	public static double timeZ(DataBot bot, double z, double holdTime){
		...
		if(z > 0 && z < s){
			double time1 = -((Math.sqrt(2 * (gravity + Constants.JUMP_ACCELERATION) * z + Math.pow(initialVelocity, 2)) + initialVelocity) / (gravity + Constants.JUMP_ACCELERATION));
			double time2 = ((Math.sqrt(2 * (gravity + Constants.JUMP_ACCELERATION) * z + Math.pow(initialVelocity, 2)) - initialVelocity) / (gravity + Constants.JUMP_ACCELERATION));
			if(time1 < 0 || Double.isNaN(time1)) return time2;
			if(time2 < 0 || Double.isNaN(time2)) return time1;
	...
	public static double maxZ(DataBot bot, double holdTime, boolean useInitialVelocity){
		Car car = bot.car;
		double gravity = treatGravity(bot);
		double initialVelocity = ((useInitialVelocity ? car.velocity.dot(car.orientation.up) : 0) + Constants.JUMP_IMPULSE);
		double s = (initialVelocity * holdTime + 0.5 * (gravity + Constants.JUMP_ACCELERATION) * Math.pow(holdTime, 2));
//		initialVelocity += (gravity * holdTime + Constants.JUMP_ACCELERATION * (holdTime - Constants.DT));
	...
	public static double treatGravity(DataBot bot){
		return bot.car.orientation.up.z > Math.cos(Math.toRadians(2)) ? bot.gravity : bot.gravity * bot.car.orientation.up.z;
